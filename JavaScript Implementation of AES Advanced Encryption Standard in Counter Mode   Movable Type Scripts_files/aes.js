...
Aes.cipher = function (input, w) { // main Cipher function [§5.1]
	...
	output = new Array(4 * Nb); // convert state to 1-d array before returning [§3.4]
	for (i = 0; i < 4 * Nb; i += 1) {
...
Aes.keyExpansion = function (key) { // generate Key Schedule (byte-array Nr+1 x Nb) from Key [§5.2]
	...
	var subWord = function (w) { // apply SBox to 4-byte word w
		for (var i = 0; i < 4; i++)
			w[i] = Aes.sBox[w[i]];
		return w;
	};
	var rotWord = function (w) { // rotate 4-byte word w left by one byte
		var tmp = w[0];
		for (var i = 0; i < 3; i++)
			w[i] = w[i + 1];
		w[3] = tmp;
...
function keyScheduleToHexStr(keySchedule) { // return expanded key as hex words, as per FIPS-197§A
	...
	for (w = 0; w < keySchedule.length; w++) {
		for (var b = 0; b < 4; b++) {
			var _byte = keySchedule[w][b];
			d += _byte < 0x10 ? '0' + _byte.toString(16) : _byte.toString(16);
		}
		d += ' ';
	}
	return d;
